              ...
              <div className="studentMenu">
                {
                  item.recommendation.length === 0 ? (<div className='studentMenu-content'>Назначать нечего</div>) : (
                    //@ts-ignore //ref={btnRef} onBlur={handleBlur} 
                    <div className="studentMenu-content">
                      <div>
                        {
                          item.recommendation.map((element: any, index: number) => (

                            <div onClick={(() => { setIndexRec(index) })} className='studentMenu-item'>
                              ...
                              <div style={{ paddingLeft: '0.5rem' }}>{element}</div>
                            </div>
                          ))
                        }
                        <hr />
                      </div>
                      <div onMouseDown={((e) => { e.preventDefault(), console.log('worked!') })} style={{ paddingTop: '0.3rem' }} className="studentMenu-define">
                        <button onClick={() => {
                          OnSubOptimization(item.recommendation[indexRec]), console.log('aboba');

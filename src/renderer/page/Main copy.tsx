          ...
          {datares.map((item: any, index: number) => (
            <ClickAwayListener onClickAway={handleTooltipClose}>
              <Tooltip
                ...
                title="Add" onClick={handleTooltipOpen}>
                <Tooltip arrow title={[
                  <div style={{ width: '13rem' }} className='tooltip-form'>
                    <p>{'Участок номер № - ' + (index + 1)}</p>
                    <p className='tooltip-avg-status'>
                      {'Состояние(среднее): ' + item.AVG}
                      <label style={{ marginLeft: '0.45rem', width: '12px', height: '12spx' }} id={'rectangle_bad'}>s</label>
                    </p>
                    <p>{'Оценка IRI: ' + item.IRI.toFixed(2)}</p>
                    <p>{'Оценка J: ' + item.J.toFixed(2)}</p>
                    <p>{'Оценка C: ' + item.C.toFixed(2)}</p>
                  </div>,
                  <hr />,
                  <p style={{ fontSize: '13px'}}>{item.recommendation}</p>
                ]}>
